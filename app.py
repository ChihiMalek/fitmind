...
if 'users_db' not in st.session_state:
    st.session_state.users_db = {
        'admin@fitmind.ai': {
            'name': 'Admin FitMind', 'initials': 'AF',
            'password': hp('Admin2024!'), 'role': 'admin', 'method': 'email'
        },
        'client@demo.com': {
            'name': 'Client Demo', 'initials': 'CD',
            'password': hp('Demo2024!'), 'role': 'client', 'method': 'email'
        }
    }

def do_login(email, pwd):
    u = st.session_state.users_db.get(email)
    if not u: return False, "Email introuvable."
    if u['password'] != hp(pwd): return False, "Mot de passe incorrect."
    return True, u

def do_register(name, email, pwd):
    if email in st.session_state.users_db: return False, "Email déjà utilisé."
    if len(pwd) < 6: return False, "Mot de passe trop court (min. 6 caractères)."
    ini = ''.join(w[0] for w in name.strip().split())[:2].upper() or name[0].upper()
    st.session_state.users_db[email] = {
        'name': name, 'initials': ini,
        'password': hp(pwd), 'role': 'client', 'method': 'email'
    }
...
def show_auth():
        ...
        if st.session_state.google_step:
            ...
            with c1:
                if st.button("✓ Confirmer", key="g_ok", use_container_width=True):
                    if '@' in gmail and len(gmail) > 5:
                        name = gmail.split('@')[0].replace('.', ' ').replace('_', ' ').title()
                        ini  = ''.join(w[0] for w in name.split())[:2].upper() or 'G'
                        if gmail not in st.session_state.users_db:
                            st.session_state.users_db[gmail] = {
                                'name': name, 'initials': ini,
                                'password': '', 'role': 'client', 'method': 'google'
                            }
                        u = st.session_state.users_db[gmail]
                        st.session_state.user = {'email': gmail, **u}
                        st.session_state.auth  = True
                        st.session_state.google_step = False
                        st.rerun()
                    else:
                        set_msg('err', 'Adresse Gmail invalide.'); st.rerun()
            with c2:
            ...
            if st.button("🔑 SE CONNECTER", key="li_btn", use_container_width=True):
                if email and pwd:
                    ok, res = do_login(email, pwd)
                    if ok:
                        st.session_state.user = {'email': email, **res}
                        st.session_state.auth  = True
                        st.rerun()
                    else:
                        set_msg('err', res); st.rerun()
                else:
    ...
    with tab_rg:
        ...
        pwd   = st.text_input("Mot de passe",    type="password", placeholder="Min. 6 caractères", key="rg_pwd")

        if st.button("✅ CRÉER MON COMPTE", key="rg_btn", use_container_width=True):
            if name and email and pwd:
                ok, msg = do_register(name, email, pwd)
                set_msg('ok' if ok else 'err', msg); st.rerun()
            else:
                set_msg('err', 'Veuillez remplir tous les champs.'); st.rerun()
...
# ══════════════════════════════════════════════════════════════════════
def prediction_form(model_reg, model_clf, scaler_reg, scaler_clf, user):
    ...
    with st.sidebar:
        ...
        <div class="sb-user-card">
            <div class="sb-avatar {'admin-ava' if role=='admin' else ''}">{initials}</div>
            <div class="sb-name">{user['name']}</div>
            <div class="sb-role {'admin-role' if role=='admin' else ''}">
                {'👑 Administrateur' if role=='admin' else '⚡ Client'}
            </div>
        </div>
        <div class="sb-section">◆ Paramètres physiologiques</div>
        """, unsafe_allow_html=True)
        ...
        st.markdown('<div class="secondary-btn">', unsafe_allow_html=True)
        if st.button("🚪 Déconnexion", key="logout_pred", use_container_width=True):
            do_logout()
        st.markdown('</div>', unsafe_allow_html=True)

    # ── Features ──
    gb = 0 if gender == "Homme" else 1
    feat = {
        'Age': age, 'Gender': gb, 'Weight (kg)': weight, 'Height (m)': height,
    ...
    if predict_btn:
        ...
        st.session_state.predictions.append({
            ...
            'calories': calories, 'level': LEVELS[ci], 'mode': 'Régression'
        })
        ...
        with st.expander("💡 Analyse et recommandations", expanded=True):
            cal_txt = ("séance légère, idéale pour la récupération." if calories < 300 else
                       "séance modérée, idéale pour maintenir la condition physique." if calories < 600 else
                       "séance intense — excellent travail cardiovasculaire !" if calories < 900 else
                       "séance de niveau athlétique — performance maximale ! 🔥")
            exp_txt = ("Concentrez-vous sur la régularité et la technique." if ci == 0 else
                       "Augmentez progressivement l'intensité et la variété." if ci == 1 else
                       "Pensez à la périodisation et à la récupération active.")
            hyd = "✅ Hydratation optimale" if water >= 2.5 else "⚠️ Hydratation insuffisante — visez 2.5L minimum."
            st.markdown(f"""
**Analyse biométrique**

- **Calories** : {calories:.0f} kcal — {cal_txt}
- **Niveau** : {EMOJIS[ci]} {LEVELS[ci]} — {exp_txt}
- **Cardio** : BPM {avg_bpm} — {"⚠️ Zone rouge, surveillez la récupération." if avg_bpm>170 else "✅ Zone cardio optimale." if avg_bpm>130 else "💡 Augmentez l'intensité pour plus d'efficacité."}
- **Hydratation** : {hyd}
            """)
    else:
        st.markdown("""
...
# CLIENT PAGE
# ══════════════════════════════════════════════════════════════════════
def show_client(model_reg, model_clf, scaler_reg, scaler_clf):
    user = st.session_state.user
    st.markdown(f"""
    <div class="main-header">
        <h1>Fit<span>Mind</span></h1>
        <p>Neural Performance Analytics — Prédiction IA</p>
        <span class="badge">⚙️ R² &gt; 0.99 &bull; 973 Athlètes &bull; Neural MLP</span>
    </div>
    <div class="user-strip">
        <span>Opérateur : <span class="u-name">{user['name']}</span> ({user.get('email','')})</span>
        <span class="u-role">CLIENT</span>
    </div>""", unsafe_allow_html=True)
    prediction_form(model_reg, model_clf, scaler_reg, scaler_clf, user)
    st.markdown('<div class="footer">FitMind · Neural Performance Analytics · Neural MLP · 973 obs.</div>', unsafe_allow_html=True)


# ══════════════════════════════════════════════════════════════════════
# ADMIN PAGE
# ══════════════════════════════════════════════════════════════════════
def show_admin(model_reg, model_clf, scaler_reg, scaler_clf):
    user  = st.session_state.user
    ...
    n_users = len(st.session_state.users_db)
    n_preds = len(preds)
    ...
    # ── UTILISATEURS ──
        ...
        rows = [{
            'Nom': u['name'], 'Email': email,
            'Rôle': '👑 Admin' if u['role']=='admin' else '👤 Client',
            'Méthode': '🔵 Google' if u.get('method')=='google' else '📧 Email'
...
if not st.session_state.auth:
    ...
    show_admin(model_reg, model_clf, scaler_reg, scaler_clf)
else:
    show_client(model_reg, model_clf, scaler_reg, scaler_clf)
