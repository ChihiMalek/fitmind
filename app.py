...
def prediction_form(model_reg, model_clf, model_wt, scaler_reg, scaler_clf, scaler_wt,
    ...
    st.markdown('<span class="sec-title">◆ Paramètres physiologiques</span>', unsafe_allow_html=True)
    ...
    with st.container(border=True):
        ...
        cimc, cconf = st.columns(2)
        with cimc:
            st.markdown(f"""<div class="imc-box" style="border-top-color:{bc}"><span class="imc-val" style="color:{bc}">{bmi:.1f}</span> <span class="imc-lbl">IMC — kg/m²</span></div>""", unsafe_allow_html=True)
        with cconf:
            st.markdown(f"""<div class="confidence-box {confidence['level']}"><span class="confidence-val">{confidence['score']:.0f}%</span> <span class="confidence-lbl">Confiance — entrées dans la distribution</span></div>""", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        if st.session_state.predictions:
            ...
            for p in reversed(st.session_state.predictions[-5:]):
                st.markdown(
                    f'<div class="hist-row"><span class="hist-badge">RÉGR.</span>'
                    f'<span class="hist-val">{p["result"]}</span>'
                    f'<span class="hist-time">{p["time"]}</span></div>',
                    unsafe_allow_html=True,
                )
...
- **Hydratation** : {reco['hydration_note']}
        ...
        <div style='background:linear-gradient(135deg,#08111E 0%,#0D1A2E 100%);
            ...
            <div style='color:var(--muted);font-size:.9rem;letter-spacing:2px;text-transform:uppercase'>
                Renseignez vos données ci-dessus<br>
