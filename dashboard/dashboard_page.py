...
from dashboard import states, layout
...
def _build_context(predictions: list, metadata: dict) -> dict:
    ...
    kpis = [
        ("🔥", f"{latest['calories']:.0f}", "Calories (kcal)"),
        ("📏", f"{latest['bmi']:.1f}", "IMC"),
        ("❤️", f"{feat.get('Max_BPM', 0):.0f}" if feat.get('Max_BPM') else "—", "BPM max"),
        ("💧", f"{latest['water']:.1f} L", "Hydratation"),
        ("🏋️", latest.get('workout_type', '—'), "Type (démo)"),
        ("🛡️", f"{confidence['score']:.0f}%", "Confiance IA"),
    ]
