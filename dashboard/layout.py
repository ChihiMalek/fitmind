...
from dashboard.components import section_header, responsive_grid


def render_ready(ctx: dict):
    """
    ctx attend les cles :
    status {priority, action, goal_progress, trend}, kpis (list of (icon,val,lbl)),
    ...
    kpi_html = [cards.kpi_card_html(icon, val, lbl) for icon, val, lbl in ctx['kpis']]
